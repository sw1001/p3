...
__kernel void setInfluence_lit(const int literals, const int size, const int dim1_size, __global int* lambdas, __global float* lambdap, __global int* dim2_size, __global float* influence){   
    ...
    if(flag < literals){
		...
		//printf("influence%d:%f\t", flag, influence[flag]);
	}
}  
